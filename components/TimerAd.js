...
function TimerAd() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setShowAd(true);
    }, 10000); // Display ad after 5 seconds
  ...
  <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
    ...
    <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
      ...
      <button onClick={handleClose} type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-bold text-gray-700 shadow-sm hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Skip Ad</button>
        <a className="flex title-font my-12 items-center md:justify-start justify-center text-gray-900"> 
        <button onClick={handleClose} type="button" class="inline-flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-bold text-white shadow-bg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">Exit</button></a>
      </div>
    </div>
  </div>
</div>
