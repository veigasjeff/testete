...
export async function getStaticPaths() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'auto.json');
    const jsonData = await fs.readFile(filePath, 'utf-8');
    const autoData = JSON.parse(jsonData);

    // Generate slugs and create paths for each auto item
    const paths = autoData.map((autoItem) => ({
      params: { id: generateSlug(autoItem.title) }, // Use the slug as the dynamic route part
    }));
...
// Fetching specific auto data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the auto item based on the slug in the URL
    const autoItem = autoData.find((item) => generateSlug(item.title) === params.id);

    if (!autoItem) {
      return { notFound: true }; // Return 404 if the auto item is not found
    }

    return {
      props: {
        autoItem, // Pass the auto item as a prop to the page
      },
    };
  } catch (error) {
    console.error("Error fetching auto item", error);
    return { notFound: true };
  }
}


const NewsSchema = (autoItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${autoItem.siteurl}/#website`,
        url: autoItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      },
      {
        ...
        datePublished: autoItem.datePublished,
        dateModified: autoItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [autoItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${autoItem.siteurl}/#article`,
        url: autoItem.siteurl,
        headline: autoItem.title,
        datePublished: autoItem.datePublished,
        dateModified: autoItem.dateModified,
        author: {
          "@type": "Person",
...
export default function autoArticle({ autoItem }) {
  const schemaData = NewsSchema(autoItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (autoItem.source && autoItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: autoItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: autoItem.source,
          },
        });
      }

      // Initialize second video player
      if (autoItem.source1 && autoItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: autoItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: autoItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
  <Head>
      <title>World News 24™ – {autoItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={autoItem.siteurl} />
        ...
        />
        <meta property="og:url" content={autoItem.siteurl} />
  ...
  <SocialSharing />
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Auto Section
        </button>
      </div>
      <h1 style={styles.title}>{autoItem.title}</h1>
      <p style={styles.date}>
        {autoItem.date} - {autoItem.time}
      </p>
      <p style={styles.courtesy}>Courtesy: {autoItem.courtesy}</p>

      {/* Description Section */}
      {autoItem.description && <p style={styles.description}>{autoItem.description}</p>}

      {/* Image Section */}
      {autoItem.image && <img src={autoItem.image} alt={autoItem.title} style={styles.image} />}

      {/* first Description */}
      {autoItem.description1 && <p style={styles.description1}>{autoItem.description1}</p>}

      {/* First YouTube Video */}
      {autoItem.source && autoItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {autoItem.image1 && <img src={autoItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {autoItem.source1 && autoItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
              {/* second Description */}
      {autoItem.description2 && <p style={styles.description2}>{autoItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(autoItem.pod || autoItem.mp3) && (
          ...
          {autoItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(autoItem.pod).searchParams.get("v")}?list=${new URL(autoItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {autoItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={autoItem.mp3} type="audio/mp3" />
