...
export async function getStaticPaths() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'business.json');
    const jsonData = await fs.readFile(filePath, 'utf-8');
    const businessData = JSON.parse(jsonData);

    // Generate slugs and create paths for each business item
    const paths = businessData.map((businessItem) => ({
      params: { id: generateSlug(businessItem.title) }, // Use the slug as the dynamic route part
    }));
...
// Fetching specific business data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the business item based on the slug in the URL
    const businessItem = businessData.find((item) => generateSlug(item.title) === params.id);

    if (!businessItem) {
      return { notFound: true }; // Return 404 if the business item is not found
    }

    return {
      props: {
        businessItem, // Pass the business item as a prop to the page
      },
    };
  } catch (error) {
...
const NewsSchema = (businessItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${businessItem.siteurl}/#website`,
        url: businessItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      {
        "@type": "WebPage",
        "@id": `${businessItem.siteurl}/#webpage`,
        url: businessItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        isPartOf: {
        ...
        datePublished: businessItem.datePublished,
        dateModified: businessItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [businessItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${businessItem.siteurl}/#article`,
        url: businessItem.siteurl,
        headline: businessItem.title,
        datePublished: businessItem.datePublished,
        dateModified: businessItem.dateModified,
        author: {
          "@type": "Person",
...
export default function businessArticle({ businessItem }) {
  const schemaData = NewsSchema(businessItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (businessItem.source && businessItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: businessItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: businessItem.source,
          },
        });
      }

      // Initialize second video player
      if (businessItem.source1 && businessItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: businessItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: businessItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
         <Head>
      <title>World News 24™ – {businessItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={businessItem.siteurl} />
        ...
        />
        <meta property="og:url" content={businessItem.siteurl} />
    ...
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Business Section
        </button>
      </div>
      <h1 style={styles.title}>{businessItem.title}</h1>
      <p style={styles.date}>
        {businessItem.date} - {businessItem.time}
      </p>
      <p style={styles.courtesy}>Courtesy: {businessItem.courtesy}</p>

      {/* Description Section */}
      {businessItem.description && <p style={styles.description}>{businessItem.description}</p>}

      {/* Image Section */}
      {businessItem.image && <img src={businessItem.image} alt={businessItem.title} style={styles.image} />}

      {/* first Description */}
      {businessItem.description1 && <p style={styles.description1}>{businessItem.description1}</p>}

      {/* First YouTube Video */}
      {businessItem.source && businessItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {businessItem.image1 && <img src={businessItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {businessItem.source1 && businessItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
          {/* second Description */}
      {businessItem.description2 && <p style={styles.description2}>{businessItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(businessItem.pod || businessItem.mp3) && (
          ...
          {businessItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(businessItem.pod).searchParams.get("v")}?list=${new URL(businessItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {businessItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={businessItem.mp3} type="audio/mp3" />
