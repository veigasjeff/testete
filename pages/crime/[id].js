...
export async function getStaticPaths() {
  try {
    ...
    const crimeData = JSON.parse(jsonData);

    // Generate slugs and create paths for each crime item
    const paths = crimeData.map((crimeItem) => ({
      params: { id: generateSlug(crimeItem.title) }, // Use the slug as the dynamic route part
    }));
    ...
    console.error("Error reading crime.json", error);
    return { paths: [], fallback: false };
  }
}

// Fetching specific crime data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the crime item based on the slug in the URL
    const crimeItem = crimeData.find((item) => generateSlug(item.title) === params.id);

    if (!crimeItem) {
      return { notFound: true }; // Return 404 if the crime item is not found
    }

    return {
      props: {
        crimeItem, // Pass the crime item as a prop to the page
      },
    };
  } catch (error) {
    console.error("Error fetching crime item", error);
    return { notFound: true };
  }
}

const NewsSchema = (crimeItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${crimeItem.siteurl}/#website`,
        url: crimeItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      },
      {
        ...
        datePublished: crimeItem.datePublished,
        dateModified: crimeItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [crimeItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${crimeItem.siteurl}/#article`,
        url: crimeItem.siteurl,
        headline: crimeItem.title,
        datePublished: crimeItem.datePublished,
        dateModified: crimeItem.dateModified,
        author: {
          "@type": "Person",
...
export default function crimeArticle({ crimeItem }) {
  const schemaData = NewsSchema(crimeItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (crimeItem.source && crimeItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: crimeItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: crimeItem.source,
          },
        });
      }

      // Initialize second video player
      if (crimeItem.source1 && crimeItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: crimeItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: crimeItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
       <Head>
      <title>World News 24™ – {crimeItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={crimeItem.siteurl} />
        ...
        />
        <meta property="og:url" content={crimeItem.siteurl} />
  ...
  <SocialSharing />
    ...
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Crime Section
        </button>
      </div>
      <h1 style={styles.title}>{crimeItem.title}</h1>
      <p style={styles.date}>
        {crimeItem.date} - {crimeItem.time}
      </p>
      <p style={styles.courtesy}>Courtesy: {crimeItem.courtesy}</p>

      {/* Description Section */}
      {crimeItem.description && <p style={styles.description}>{crimeItem.description}</p>}

      {/* Image Section */}
      {crimeItem.image && <img src={crimeItem.image} alt={crimeItem.title} style={styles.image} />}

      {/* first Description */}
      {crimeItem.description1 && <p style={styles.description1}>{crimeItem.description1}</p>}

      {/* First YouTube Video */}
      {crimeItem.source && crimeItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {crimeItem.image1 && <img src={crimeItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {crimeItem.source1 && crimeItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
         {/* second Description */}
         {crimeItem.description2 && <p style={styles.description2}>{crimeItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(crimeItem.pod || crimeItem.mp3) && (
          ...
          {crimeItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(crimeItem.pod).searchParams.get("v")}?list=${new URL(crimeItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {crimeItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={crimeItem.mp3} type="audio/mp3" />
