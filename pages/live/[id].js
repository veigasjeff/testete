...
export async function getStaticPaths() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'live.json');
    const jsonData = await fs.readFile(filePath, 'utf-8');
    const liveData = JSON.parse(jsonData);

    // Generate slugs and create paths for each live item
    const paths = liveData.map((liveItem) => ({
      params: { id: generateSlug(liveItem.title) }, // Use the slug as the dynamic route part
    }));
    ...
    console.error("Error reading live.json", error);
    return { paths: [], fallback: false };
  }
}

// Fetching specific live data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the live item based on the slug in the URL
    const liveItem = liveData.find((item) => generateSlug(item.title) === params.id);

    if (!liveItem) {
      return { notFound: true }; // Return 404 if the live item is not found
    }

    return {
      props: {
        liveItem, // Pass the live item as a prop to the page
      },
    };
  } catch (error) {
...
const NewsSchema = (liveItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${liveItem.siteurl}/#website`,
        url: liveItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      },
      {
        ...
        datePublished: liveItem.datePublished,
        dateModified: liveItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [liveItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${liveItem.siteurl}/#article`,
        url: liveItem.siteurl,
...
export default function liveArticle({ liveItem }) {
  const schemaData = NewsSchema(liveItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (liveItem.source && liveItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: liveItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: liveItem.source,
          },
        });
      }

      // Initialize second video player
      if (liveItem.source1 && liveItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: liveItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: liveItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
       <Head>
      <title>World News 24™ – {liveItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={liveItem.siteurl} />
        ...
        />
        <meta property="og:url" content={liveItem.siteurl} />
  ...
  <SocialSharing />
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Live Section
        </button>
      </div>
      <h1 style={styles.title}>{liveItem.title}</h1>
      {/* <p style={styles.date}>
        {liveItem.date} - {liveItem.time}
      </p>
      <p style={styles.courtesy}>Courtesy: {liveItem.courtesy}</p> */}

      {/* Description Section */}
      {liveItem.description && <p style={styles.description}>{liveItem.description}</p>}

      {/* Image Section */}
      {liveItem.image && <img src={liveItem.image} alt={liveItem.title} style={styles.image} 
      
      />}

      {/* first Description */}
      {liveItem.description1 && <p style={styles.description1}>{liveItem.description1}</p>}

      {/* First YouTube Video */}
      {liveItem.source && liveItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {liveItem.image1 && <img src={liveItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {liveItem.source1 && liveItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
  {/* second Description */}
  {liveItem.description2 && <p style={styles.description2}>{liveItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(liveItem.pod || liveItem.mp3) && (
          ...
          {liveItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(liveItem.pod).searchParams.get("v")}?list=${new URL(liveItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {liveItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={liveItem.mp3} type="audio/mp3" />
