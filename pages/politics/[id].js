...
export async function getStaticPaths() {
  try {
    ...
    // Generate slugs and create paths for each politics item
    const paths = politicsData.map((politicsItem) => ({
      params: { id: generateSlug(politicsItem.title) }, // Use the slug as the dynamic route part
    }));
    ...
    console.error("Error reading politics.json", error);
    return { paths: [], fallback: false };
  }
}

// Fetching specific politics data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the politics item based on the slug in the URL
    const politicsItem = politicsData.find((item) => generateSlug(item.title) === params.id);

    if (!politicsItem) {
      return { notFound: true }; // Return 404 if the politics item is not found
    }

    return {
      props: {
        politicsItem, // Pass the politics item as a prop to the page
      },
    };
  } catch (error) {
    console.error("Error fetching politics item", error);
    return { notFound: true };
  }
}


const NewsSchema = (politicsItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${politicsItem.siteurl}/#website`,
        url: politicsItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      {
        "@type": "WebPage",
        "@id": `${politicsItem.siteurl}/#webpage`,
        url: politicsItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        isPartOf: {
        ...
        datePublished: politicsItem.datePublished,
        dateModified: politicsItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [politicsItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${politicsItem.siteurl}/#article`,
        url: politicsItem.siteurl,
        headline: politicsItem.title,
        datePublished: politicsItem.datePublished,
        dateModified: politicsItem.dateModified,
        author: {
          "@type": "Person",
...
export default function politicsArticle({ politicsItem }) {
  const schemaData = NewsSchema(politicsItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (politicsItem.source && politicsItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: politicsItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: politicsItem.source,
          },
        });
      }

      // Initialize second video player
      if (politicsItem.source1 && politicsItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: politicsItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: politicsItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
       <Head>
      <title>World News 24™ – {politicsItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={politicsItem.siteurl} />
        ...
        />
        <meta property="og:url" content={politicsItem.siteurl} />
    ...
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Politics Section
        </button>
      </div>
      <h1 style={styles.title}>{politicsItem.title}</h1>
      <p style={styles.date}>
        {politicsItem.date} - {politicsItem.time}
      </p>
      <p style={styles.courtesy}>Courtesy: {politicsItem.courtesy}</p>

      {/* Description Section */}
      {politicsItem.description && <p style={styles.description}>{politicsItem.description}</p>}

      {/* Image Section */}
      {politicsItem.image && <img src={politicsItem.image} alt={politicsItem.title} style={styles.image} />}

      {/* first Description */}
      {politicsItem.description1 && <p style={styles.description1}>{politicsItem.description1}</p>}

      {/* First YouTube Video */}
      {politicsItem.source && politicsItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {politicsItem.image1 && <img src={politicsItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {politicsItem.source1 && politicsItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
          {/* second Description */}
      {politicsItem.description2 && <p style={styles.description2}>{politicsItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(politicsItem.pod || politicsItem.mp3) && (
          ...
          {politicsItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(politicsItem.pod).searchParams.get("v")}?list=${new URL(politicsItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {politicsItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={politicsItem.mp3} type="audio/mp3" />
