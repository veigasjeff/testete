...
export async function getStaticPaths() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'sports.json');
    const jsonData = await fs.readFile(filePath, 'utf-8');
    const sportsData = JSON.parse(jsonData);

    // Generate slugs and create paths for each sports item
    const paths = sportsData.map((sportsItem) => ({
      params: { id: generateSlug(sportsItem.title) }, // Use the slug as the dynamic route part
    }));
...
// Fetching specific sports data based on the dynamic slug (id)
export async function getStaticProps({ params }) {
  try {
    ...
    // Find the sports item based on the slug in the URL
    const sportsItem = sportsData.find((item) => generateSlug(item.title) === params.id);

    if (!sportsItem) {
      return { notFound: true }; // Return 404 if the sports item is not found
    }

    return {
      props: {
        sportsItem, // Pass the sports item as a prop to the page
      },
    };
  } catch (error) {
...
const NewsSchema = (sportsItem) => 
  JSON.stringify({
    ...
    "@graph": [
      ...
      {
        "@type": "WebSite",
        "@id": `${sportsItem.siteurl}/#website`,
        url: sportsItem.siteurl,
        name: "World News 24™ – Stay Informed, Stay Ahead.",
        publisher: {
          "@type": "Organization",
          "@id": "https://worldsnews24.vercel.app/#organization"
      ...
      },
      {
        ...
        datePublished: sportsItem.datePublished,
        dateModified: sportsItem.dateModified,
        breadcrumb: {
        ...
        potentialAction: [
          {
            "@type": "ReadAction",
            target: [sportsItem.siteurl]
          }
        ]
      },
      {
        "@type": "Article",
        "@id": `${sportsItem.siteurl}/#article`,
        url: sportsItem.siteurl,
        headline: sportsItem.title,
        datePublished: sportsItem.datePublished,
        dateModified: sportsItem.dateModified,
        author: {
          "@type": "Person",
...
export default function sportsArticle({ sportsItem }) {
  const schemaData = NewsSchema(sportsItem); 
  ...
  useEffect(() => {
    ...
    loadYouTubeAPI().then(() => {
      ...
      if (sportsItem.source && sportsItem.source !== "#") {
        new window.YT.Player("player-0", {
          videoId: sportsItem.source,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: sportsItem.source,
          },
        });
      }

      // Initialize second video player
      if (sportsItem.source1 && sportsItem.source1 !== "#") {
        new window.YT.Player("player-1", {
          videoId: sportsItem.source1,
          playerVars: {
            playsinline: 1,
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: sportsItem.source1,
          },
        });
      }
    });
  ...
  return (
    <>
    <Head>
      <title>World News 24™ – {sportsItem.title || "Default Title"}</title>
        ...
        <meta
          ...
          content="World News 24™: Your trusted source for breaking news, global updates, and real-time coverage of politics, business, technology, culture, sports, and more—24/7."
        />
        <link rel="canonical" href={sportsItem.siteurl} />
        ...
        />
        <meta property="og:url" content={sportsItem.siteurl} />
  ...
  <SocialSharing />
    ...
    <div style={styles.container}>
        ...
        <button onClick={goBackToMain} style={styles.pageButton}>
          Back to Main Sports Section
        </button>
      </div>
      <h1 style={styles.title}>{sportsItem.title}</h1>
      <p style={styles.date}>
       Date: {sportsItem.date} - Time {sportsItem.time}
      
      </p>
      <p style={styles.courtesy}>Courtesy: {sportsItem.courtesy}</p>

      {/* Description Section */}
      {sportsItem.description && <p style={styles.description}>{sportsItem.description}</p>}

      {/* Image Section */}
      {sportsItem.image && <img src={sportsItem.image} alt={sportsItem.title} style={styles.image} />}

      {/* first Description */}
      {sportsItem.description1 && <p style={styles.description1}>{sportsItem.description1}</p>}

      {/* First YouTube Video */}
      {sportsItem.source && sportsItem.source !== "#" && (
      ...
      {/* Image 1 Section */}
      {sportsItem.image1 && <img src={sportsItem.image1} alt="Additional" style={styles.image} />}

      {/* Second YouTube Video */}
      {sportsItem.source1 && sportsItem.source1 !== "#" && (
        <div style={styles.source}>
          <div id="player-1" style={styles.youtubePlayer}></div>
        </div>
      )}
           {/* second Description */}
      {sportsItem.description2 && <p style={styles.description2}>{sportsItem.description2}</p>}
      {/* Embed MP3 Player and Podcast Indicator */}
      {(sportsItem.pod || sportsItem.mp3) && (
          ...
          {sportsItem.pod && (
            <div style={styles.podcastWrapper}>
              ...
              <iframe
                width="50%"
                height="80"
                src={`https://www.youtube.com/embed/${new URL(sportsItem.pod).searchParams.get("v")}?list=${new URL(sportsItem.pod).searchParams.get("list")}&controls=1`}
                frameBorder="0"
          ...
          {sportsItem.mp3 && (
            <div style={styles.audioWrapper}>
              ...
              {/* <audio controls style={styles.audioPlayer}>
                <source src={sportsItem.mp3} type="audio/mp3" />
